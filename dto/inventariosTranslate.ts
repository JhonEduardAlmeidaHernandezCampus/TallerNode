...
export class putInventarios {
    ...
    @Transform(({value}) => {if(Math.floor(value) && typeof value == "number") return Math.floor(value); else throw {status: 200, message: `Error en los parametros`};}, {toClassOnly: true})
    id_producto: number;
